...
#[test]
fn toml_navigation_animation_speed_is_independent_from_camera_speed() {
    let toml = r#"
        [navigation]
        animation_speed = 0.8
        camera_speed = 0.4
    ...
    assert!(
        (config.camera_speed - 0.4).abs() < f64::EPSILON,
        "camera_speed should retain its independent value"
    );
    assert!((config.animation_speed - 0.8).abs() < f64::EPSILON);
